...
class Hasil extends CI_Controller{
   ...
   function index($notice = FALSE,  $gel_id = FALSE, $hapus_cari = FALSE){ //, $jur_id = FALSE, $cari = FALSE
      ...
      foreach ($cek_gel_ta as $key => $row){
         ...
         $data['filter_gel'][$row['gel_id']] = $batas.anchor('u/hasil/index/false/'.encode($row['gel_id']).'/do', $link_text);
      }
      // input
      ...
      $data['post_jur'] = 'u/hasil/index/false/'.encode($gel_id);
      // search
      ...
      $data['post_cari'] = 'u/hasil/index/false/'.encode($gel_id);
      $data['data_cari'] = $cari;
      //jenis test
      ...
      if($query->num_rows == 0 AND strlen($cari) > 0) $data['notice'] = array('yellow', 'Hasil pencarian untuk <b>"'.$cari.'"</b> tidak ditemukan!');
      //elseif($jur_id == 0) //$data['notice'] = array('red', 'ERROR : Jurusan belum memiliki kuota! '.anchor('u/kuota/add', 'Tambah Kuota'));
      elseif($query->num_rows == 0) $data['notice'] = array('red', 'Hasil seleksi belum ada!');
      //hasil akhir
      ...
      if($this->agent->is_mobile()) $this->load->view(view('u', 'view_hasil_mobile'), $data);
      else $this->load->view(view('u', 'view_hasil'), $data);
   }
}