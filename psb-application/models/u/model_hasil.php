...
class Model_hasil extends CI_Model{
   function index($data){
      ...
      $query = 'SELECT siswa.*, pilihan.*, nilai_uan, prestasi.*, '.$test_kolom.',
         ...
         JOIN (SELECT * FROM (SELECT * FROM pilihan WHERE pilihan_status IN ("diterima", "cadangan") AND pilihan_jur = '.$data[1].' ORDER BY pilihan_siswa ASC, pilihan_ke DESC) AS hasil GROUP BY pilihan_siswa) AS pilihan ON pilihan_siswa = siswa_id
